...
/**
 * IXS adapter layer (BNB Chain, production).
 *
 * READ  — vault registry (IXS Vault API + contract reads), catalog, availability of announced products.
 * WRITE — approve + deposit calldata via the IXS MCP (`vault_build_request_deposit`). A local ERC-4626 encoder
 *         exists only as a fallback when the MCP is unreachable, and every step records which one built it.
...
/**
 * Live availability check against the IXS Vault API: a strategy is available when its vault (routeId) or a vault
 * for its asset on this chain exists. Announced products without a vault are not.
 */
export async function vaultAvailability(strategies: VaultStrategy[]): Promise<Record<string, { available: boolean; detail: string }>> {
  ...
  for (const s of strategies) {
    const match = live.items.find((v) => (s.routeId ? v.routeId === s.routeId : v.chainId === CHAIN_ID && (v.underlyingAsset?.symbol ?? "").toUpperCase().includes(s.asset.toUpperCase())));
    const anywhere = !match && live.items.find((v) => (v.underlyingAsset?.symbol ?? "").toUpperCase().includes(s.asset.toUpperCase()));
    out[s.id] = match
      ? { available: true, detail: `${s.vaultName}: live on the IXS Vault API (${match.name}, ${match.chainName ?? match.network})` }
      : {
          available: false,
          detail: !live.ok
            ? `${s.vaultName}: IXS Vault API unreachable, availability unknown`
            : anywhere
              ? `${s.vaultName}: no ${s.asset} vault on ${CHAIN_NAME} (IXS lists one on ${anywhere.chainName ?? anywhere.network})`
              : `${s.vaultName}: announced by IXS, no ${s.asset} vault deployed on the IXS Vault API`,
        };
  }
  return out;
}
...
/** Catalog built from the registry (addresses from the IXS API, state from the contracts) plus availability data. */
export async function getStrategies(): Promise<StrategiesResult> {
  ...
  const liveVaults: LiveVaultSummary[] = live.items.map((v) => ({
    ...
    symbol: v.symbol,
    chainId: v.chainId,
    chainName: v.chainId === CHAIN_ID ? CHAIN_NAME : v.chainId === 43114 ? "Avalanche C-Chain" : (v.chainName ?? v.network),
 ...
 * Built by the IXS MCP; the local ERC-4626 encoder is used only when the MCP is unreachable and the caller allows it.
 */
export async function buildDepositSteps(
  ...
  if (strategy.routeId) {
    try {
      ...
      return {
        ...
        steps: plan.steps!.map((s) => {
          ...
          return {
            ...
            value: s.tx.value ?? "0",
            description: isApprove ? `Approve ${strategy.asset} for ${strategy.vaultName} (IXS MCP)` : `${kind === "requestDeposit" ? "Request deposit of" : "Deposit"} ${strategy.asset} into ${strategy.vaultName} (IXS MCP)`,
      ...
      // The MCP refusing (limit 0, not whitelisted, paused) is a real answer: surface it instead of encoding locally.
      if (!opts.allowLocalFallback || /limit|whitelist|paused|exceeds/i.test(mcpError)) throw new Error(mcpError);
    }
  }
  ...
  return {
    ...
    note: mcpError ? `IXS MCP unavailable (${mcpError}); calldata encoded locally with the standard ERC-4626 ABI` : undefined,
    steps: [
      { ...base, kind: "approve", to: asset, data: approve, value: "0", description: `Approve ${strategy.asset} for ${strategy.vaultName}`, builtBy: "local-encoder" },
      {
        ...base,
