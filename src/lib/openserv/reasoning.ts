...
export function localMemo(i: NarrativeInput): Memo {
  ...
  return {
    ...
    sections: [
      ...
      { heading: "Policy", body: `Liquidity floor ${user.liquidityFloorPct}%, max single-asset exposure ${user.maxAssetExposurePct}%, minimum vault risk score ${user.minVaultRiskScore}, monthly burn ${fmtUsd(user.monthlyBurnUsd)}. Minimum deposit 100 USDC per request (confirmed by IXS). ${i.policyChecks.map((c) => `${c.label}: ${c.detail}`).join("; ")}.` },
