...
| 0:10–0:22 | Home: total treasury about $2.0M, about $1.6M idle for 23 days (BTC and USDC). Click **Run analysis**; the agent steps tick through. | "Acme DAO is a simulated treasury: about 1.6 million dollars has sat idle for 23 days. One click runs the pipeline: scanner, opportunity finder, the Risk Guardian's pre-flight on every vault, the planner's caps, then SERV reasoning." |
...
| 0:31–0:43 | **Avalanche open vault · DEFER**. Expand its pre-flight: deposit limit 0, NAV from 14 September, contract threshold exceeded. | "The Avalanche vault is deferred, not rejected. Its deposit limit is zero because the NAV is stale, and IXS told participants that a zero limit means temporarily paused, waiting for a NAV refresh. The watcher flags it the moment it reopens." |
| 0:43–0:51 | **Licensed vaults (BNB and Avalanche) · REJECT**. | "The licensed vaults are rejected: they need KYC whitelisting, and the IXS MCP confirms this wallet is not on the list." |
| 0:51–1:04 | **BNB open vault ixv1 · ALLOCATE**, amount about 225,000 USDC. Show the guardrails line and "0 validator overrides". | "The open BNB vault passes every check, so SERV allocates about 225,000 USDC within the planner's cap, keeping the treasury well above its 30 percent liquidity floor. The guardrails needed zero overrides: SERV stayed inside the limits on its own." |
| 1:04–1:19 | **Approve & execute (Simulate)**. Modal: "Simulated on BNB mainnet", approve exact + deposit built by the IXS MCP, eth_call with a state override, expected shares (about 206,000 ixv1), gas. Run it; the steps turn green. | "Approve runs the exact calldata the IXS MCP built, approve and deposit, through eth_call on BNB mainnet with a state override for the balance and allowance. We get the expected shares and the gas, straight from the real contract. Nothing is sent, and a judge can repeat this from an empty wallet." |
| 1:19–1:31 | Terminal: fork run output, "Mainnet fork (block N)", approve and deposit confirmed on the fork, shares minted, then `requestRedeem` queued. | "On an Anvil fork of mainnet the same calldata really executes: 104 USDC mints about 95 ixv1, and the redemption request is queued for the operator. Why 104 and not 100? Redeeming a 100 USDC position returns 99.5 after the fee, below the vault's 100 USDC minimum. So Live deposits have a 104 USDC floor." |
...
| 1:43–1:55 | `/evidence`: statements from IXS and the judges (dated), vault table with limits, NAV timestamps and blocks, the committed snapshot with SERV input and output, fork runs, **Export JSON**. | "Everything is on a public evidence page: every IXS call, every on-chain read with its block, SERV's exact input and output, and the fork runs. Live mode is built and capped, but this submission is simulated. Vaulto: SERV decides, guardrails enforce, and the evidence is public." |
