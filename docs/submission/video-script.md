...
| 0:31–0:43 | **Avalanche open vault · DEFER**. Expand its pre-flight: deposit limit 0, NAV from 14 September, older than Vaulto's 72-hour policy. | "The Avalanche vault is deferred, not rejected. Its deposit limit is zero while its NAV is over ten days old. IXS stated that a zero limit relates to NAV staleness, and Vaulto's policy is to defer such a vault until the NAV is refreshed. The watcher flags it the moment it reopens." |
...
## If ixv1 is deferred at recording time: use Replay

Switch the dashboard toggle to **Replay** (default block 123,779,792). Every view then shows "Replay: BNB mainnet state @ block 123779792 (NAV fresh at that block)", and the ALLOCATE, Simulate and memo scenes above work as written, at that block. Say it once in the voice-over at 0:51: "This is a replay of the BNB mainnet state at block 123,779,792, when the ixv1 NAV was fresh; today the vault is waiting for a NAV refresh, which the current view shows as a deferral." Record the Current view first (DEFER / REJECT today), then the Replay view.

The repository's nav-watch workflow opens an issue (and emails the repository owner) as soon as ixv1 accepts deposits again, if you would rather record on the current state.
