...
export async function runPreflight(v: RegistryVault, wallet: string, amountUsd?: number): Promise<VaultPreflight> {
  ...
  checks.push({
    ...
    detail: age == null
      ...
      : `${navOk ? "within" : "older than"} the Vaulto staleness policy of ${env.navStaleHours} h (IXS publishes no navStalenessThreshold). Last NAV change ${v.nav.updatedAt ? new Date(v.nav.updatedAt * 1000).toISOString() : "?"}${v.nav.block ? ` at block ${v.nav.block}` : ""}${v.nav.lastChangeTx ? ` (tx ${v.nav.lastChangeTx.slice(0, 12)}…)` : ""}. ${navOk ? "" : "Per IXS (24 Sep 2026) a stale NAV drives the deposit limit to 0 until the next refresh: temporarily paused, waiting NAV refresh."}`,
    source: v.nav.source,
  });
