...
export async function chatCompletion(params: { messages: ChatMessage[]; model?: string; temperature?: number; timeoutMs?: number }): Promise<ChatResult> {
...
export async function checkInference(): Promise<{ ok: boolean; model?: string; error?: string }> {
  try {
    const r = await chatCompletion({ messages: [{ role: "user", content: "Reply with OK." }], timeoutMs: 15_000 });
